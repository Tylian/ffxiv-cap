        ...
        c.on('packet', (nbytes, trunc) => {
            ...
            if(eth.info.type === PROTOCOL.ETHERNET.IPV4) {
                ...
                if(ip.info.protocol === PROTOCOL.IP.TCP) {
                    ...
                    let id = `${ip.info.srcaddr} ${ip.info.dstaddr} ${tcp.info.srcport} ${tcp.info.dstport}`;

                    if(dataBuffer.includes(FFXIV_HEADER)) {
                        console.log(incoming ? '<-' : '->', 'FFXIV packet!', tcp.info.seqno);
                        streams[id] = true;
                    } else if(streams[id]) {
                        console.log(incoming ? '<-' : '->', 'Non-FFXIV packet in FFXIV Seq!', tcp.info.seqno);
                    }
